...
async def process_topic(request: PromptRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        # ── Phase 1: DB + keyword generation run IN PARALLEL ─
        # No more sleep(11) — we await both together, then decide what to do
        ...
        # ── Phase 2: Web scrape (news OR deep) ───────────────
        if len(db_results) >= 3:
            ...
            print(f"--- DB MISS: Deep web scrape with keywords: {base_keywords} ---")
            new_articles = await deep_search_and_scrape(base_keywords, scraped_urls)

        # ── Phase 3: Merge context ────────────────────────────
